...
class Feedback(DeptAACMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['rpt'] = self.report
        context['gRub'] = self.report.rubric
        context['gri1'] = self.GRIs.filter(item__section=1).order_by("item__order","pk")
        context['gri2'] = self.GRIs.filter(item__section=2).order_by("item__order","pk")
        context['gri3'] = self.GRIs.filter(item__section=3).order_by("item__order","pk")
        context['gri4'] = self.GRIs.filter(item__section=4).order_by("item__order","pk")
        context = section1Context(self,context)
        context = section2Context(self,context)
        context = section3Context(self,context)
        context = section4Context(self,context)
